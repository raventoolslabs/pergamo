...
describe('Listing endpoints', () => {
  ...
  beforeAll(async () => {
    ...
    await api.put(`/api/document/${documentId}`, { tags: ['listado'] }, {
  ...
  it('Should list the documents of the authenticated organization', async () => {
    ...
    const response = await api.get('/api/document', { headers: { authorization: tokenOwner } });
  ...
  it('Should not list documents from another organization', async () => {
    ...
    const response = await api.get('/api/document', { headers: { authorization: tokenOther } });
  ...
  it('Should filter by name, tag and scan status', async () => {
    ...
    let response = await api.get(`/api/document?name=${encodeURIComponent(documentName)}`, {
      headers: { authorization: tokenOwner }
    });
    expect(response.status).toBe(StatusCodes.OK);
    expect(response.data.documents.some((item:any) => item.id === documentId)).toBe(true);

    response = await api.get('/api/document?tag=listado', { headers: { authorization: tokenOwner } });
    expect(response.status).toBe(StatusCodes.OK);
    expect(response.data.documents.some((item:any) => item.id === documentId)).toBe(true);

    response = await api.get('/api/document?tag=inexistente', { headers: { authorization: tokenOwner } });
    expect(response.status).toBe(StatusCodes.OK);
    expect(response.data.documents.length).toBe(0);

    response = await api.get('/api/document?scan_status=infected', { headers: { authorization: tokenOwner } });
    expect(response.status).toBe(StatusCodes.OK);
    expect(response.data.documents.some((item:any) => item.id === documentId)).toBe(false);
  });
  ...
  it('Should accept several scan statuses at once', async () => {
    ...
    let response = await api.get('/api/document?scan_status=infected,error', {
      headers: { authorization: tokenOwner }
    });
    expect(response.status).toBe(StatusCodes.OK);
    expect(response.data.documents.some((item:any) => item.id === documentId)).toBe(false);

    response = await api.get('/api/document?scan_status=clean,pending', {
      headers: { authorization: tokenOwner }
    });
    expect(response.status).toBe(StatusCodes.OK);
    expect(response.data.documents.some((item:any) => item.id === documentId)).toBe(true);
  ...
  it('Should filter by deposit date range', async () => {
    ...
    let response = await api.get(`/api/document?from=${encodeURIComponent(pasado)}&to=${encodeURIComponent(futuro)}`, {
      headers: { authorization: tokenOwner }
    });
    expect(response.status).toBe(StatusCodes.OK);
    expect(response.data.documents.some((item:any) => item.id === documentId)).toBe(true);
    ...
    response = await api.get(`/api/document?to=${encodeURIComponent(pasado)}`, {
      headers: { authorization: tokenOwner }
    });
    expect(response.status).toBe(StatusCodes.OK);
    expect(response.data.documents.some((item:any) => item.id === documentId)).toBe(false);

    response = await api.get(`/api/document?from=${encodeURIComponent(futuro)}`, {
      headers: { authorization: tokenOwner }
    });
    expect(response.status).toBe(StatusCodes.OK);
    expect(response.data.documents.some((item:any) => item.id === documentId)).toBe(false);
  ...
  it('Should paginate', async () => {
    ...
    const response = await api.get('/api/document?limit=1&offset=0', { headers: { authorization: tokenOwner } });
  ...
  it('Should sort by name and by scan status severity', async () => {
    ...
    const names = async (query:string) => {
      const response = await api.get(`/api/document?name=sortcase&${query}`, {
        headers: { authorization: tokenOwner }
      });
  ...
  it('Should reject invalid query parameters instead of ignoring them', async () => {
    ...
    for(const query of ['limit=0', 'limit=1000', 'offset=-1', 'sort=path', 'order=random',
      ...
      'to=ayer', 'unknown=1']) {
      const response = await api.get(`/api/document?${query}`, { headers: { authorization: tokenOwner } });
  ...
  it('Should not treat wildcards in the name filter as patterns', async () => {
    ...
    // caracter literal.
    const response = await api.get('/api/document?name=%25', { headers: { authorization: tokenOwner } });
  ...
  it('Should tell a master token that it has no organization', async () => {
    ...
    const response = await api.get('/api/document', { headers: { authorization: tokenMaster } });
  ...
  it('Should expose the scan state of a document', async () => {
    ...
    const response = await api.get(`/api/document/${documentId}/scan`, {
      headers: { authorization: tokenOwner }
    });
  ...
  it('Should list organizations for the master user', async () => {
    ...
    const response = await api.get('/api/organization', { headers: { authorization: tokenMaster } });
  ...
  it('Should filter organizations by name', async () => {
    ...
    const response = await api.get(`/api/organization?name=${OTHER_ORGANIZATION}`, {
      headers: { authorization: tokenMaster }
    });
  ...
  it('Should refuse to list organizations without the master token', async () => {
    ...
    let response = await api.get('/api/organization', { headers: { authorization: tokenOwner } });
    expect(response.status).toBe(StatusCodes.UNAUTHORIZED);

    response = await api.get('/api/organization');
    expect(response.status).toBe(StatusCodes.UNAUTHORIZED);
  });
  ...
  it('Should serve the deployment limits to an authenticated client', async () => {
    ...
    const response = await api.get('/api/config', { headers: { authorization: tokenOwner } });
